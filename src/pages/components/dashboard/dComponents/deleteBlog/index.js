...
async function fetchBlogData() {
  ...
  try {
    const res = await fetch("https://code-shine-technology.vercel.app/blog");
...
const remove = () => {
  ...
  const handleDelete = async (id) => {
    console.log(id);
    try {
      await fetch(`https://code-shine-technology.vercel.app/blog/${id}`, {
        method: "DELETE",
      })
              ...
              <>
                <div className="flex justify-between bg-indigo-100 mb-5 md:w-3/4 w-full mx-auto">
                  ...
                  <button className="btn btn-outline ml-2 my-auto">
                    <Link href={`/components/dashboard/dComponents/deleteBlog/${blog._id}`}>
